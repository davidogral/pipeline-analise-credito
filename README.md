...
trata e valida os dados em uma **arquitetura medalhão (Bronze → Silver → Gold)** persistida em **Parquet**, bloqueia
a publicação se as regras de qualidade falharem, carrega um modelo relacional no **PostgreSQL** e é orquestrado
diariamente pelo **Apache Airflow**. Sobre a camada Gold, consultas em **Spark SQL** e um modelo **`pyspark.ml`**
estimam o score de crédito.
...
**PySpark** (DataFrame API, Spark SQL, `pyspark.ml`) · **Parquet** · **SQL** (Spark SQL, PostgreSQL) ·
**Apache Airflow** (TaskFlow API) · **Docker Compose** · **pytest** · **GitHub Actions** · **Ruff**
...
- **Parquet entre as camadas.** Formato colunar e com schema embutido: a Silver não precisa inferir tipos da
  Bronze, e a estrutura está pronta para evoluir para **Delta Lake**.
...
- **Cache e particionamento conscientes.** A Gold é cacheada porque alimenta três agregações; as agregações
  pequenas são gravadas com `coalesce(1)` para não gerar dezenas de arquivos minúsculos.
...
- Rodar no **Databricks**, trocando Parquet por **Delta Lake** (`MERGE` para carga incremental e time travel).
- Carga incremental por `CODIGO_CLIENTE` em vez de recriar as tabelas.
